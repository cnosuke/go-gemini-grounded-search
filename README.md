...
func main() {
	...
	// By default, it will use a model like "gemini-3-pro-preview" (see constants.go)
...
func main() {
	...
	client, err := search.NewClient(
		ctx,
		apiKey,
		search.WithModelName("gemini-3-pro-preview"),           // Use a specific model (see options.go)
...
- `WithModelName(name string)`: Specifies which Gemini model to use (e.g., `"gemini-3-pro-preview"`).

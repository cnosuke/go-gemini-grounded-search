...
func main() {
	...
	// By default, it will use "gemini-3.8-flash" (see constants.go).
...
client, err := search.NewClient(
    ctx,
    "your-api-key",
    search.WithModelName("gemini-3.1-pro-preview"), // or "gemini-3.8-flash" for faster/cheaper
...
- `WithModelName(name string)`: Specifies which Gemini model to use (e.g., `"gemini-3.8-flash"` or `"gemini-3.1-pro-preview"`).
...
- `WithDefaultThinkingConfig(tc *ThinkingConfig)`: Controls the model's thinking behavior. For Gemini 3.x series models, use `ThinkingLevel` (`ThinkingLevelMinimal`, `ThinkingLevelLow`, `ThinkingLevelMedium`, `ThinkingLevelHigh`). For Gemini 2.5 series models, use `ThinkingBudget` (set to `0` to disable thinking).
